    ...
    for device in local.devices : [
      for acl in lookup(local.config, "acls", []) : [
        for entry in lookup(acl, "entries", []) : {
          ...
          remark                    = lookup(entry, "remark", null)
          ack                       = lookup(entry, "ack", false)
          rst                       = lookup(entry, "rst", false)
          destination_address_group = lookup(entry, "destination_address_group", null) != null ? "eq" : null
        }]
    ]
  ]) : e.key => e }
}
...
resource "nxos_ipv4_access_list_entry" "acl_entry" {
  ...
  ack                       = each.value.ack
  rst                       = each.value.rst
  destination_address_group = each.value.destination_address_group
